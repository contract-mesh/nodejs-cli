...
async function validate(context: ContractContext) {
  ...
  if (results.length === 0) {
    // features
    if (context.element.features) {
      const featureKeys = Object.keys(context.element.features);

      for (let i = 0; i < featureKeys.length; i++) {
        const featureKey = featureKeys[i];

        const featureContext = context.child(context.element.features[featureKey], `features['${featureKey}']`, 'feature', featureKey);
    ...
    // dependencies
    if (context.element.dependencies) {
      const projectNames = Object.keys(context.element.dependencies);
      ...
      for (let i = 0; i < projectNames.length; i++) {
        ...
        if (dependencyProject == null) {
          ...
          const dependencies = context.element.dependencies[projectName];
